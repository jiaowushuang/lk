...
#define IDLE_STACK_ALIGHMENT 16

/* the idle thread(s) (statically allocated) */
#if WITH_SMP
static thread_t _idle_threads[SMP_MAX_CPUS];
char _idle_stack[SMP_MAX_CPUS][IDLE_STACK_SIZE] __ALIGNED(IDLE_STACK_ALIGHMENT);
#define idle_thread(cpu) (&_idle_threads[cpu])
#else
static thread_t _idle_thread;
char _idle_stack[IDLE_STACK_SIZE] __ALIGNED(IDLE_STACK_ALIGHMENT);
...
#if defined(ARCH_HAS_MPU) && defined(CONFIG_MULTIPARTITIONING)
        z_mem_domain_exit_thread(current_thread);
#endif
    } else {
...
/* 在idle任务中由中断句柄所调用的 <thread_preempt> 函数进行任务切换
 * idle_thread_routine {
 *    ...   中断向量->异步句柄:
 *          异常上下文保存 <regsave_long/short>
 *          irq handler {
 *                  ...
 *                  thread_preempt
 *                      加线程局部锁（旧任务） THREAD_LOCK(state);
 *                          ...
 *                          保护的线程数据结构
 *                      thread_resched {
 *                          此处切换到别的任务或当前任务
 ...
 *                      恢复执行点
 *                          LR记录的是被切换时所在函数的下一指令， 
 *                          故凡是调用 <thread_resched> 函数的，
 *                          都会在此调度点切换到别的任务或当前任务
 *                      解线程局部锁（新任务或当前 - 交叉结构） THREAD_UNLOCK(state);
 *                          当新线程退出时将不会再调度回来，故无需解锁，但仍会关闭中断      
 *          }
 *          异常上下文恢复 <regrestore_long/short>
 * }
 * 
 * 与idle_thread_routine类似的是其他用户和内核任务的例程，
 * 在执行系统调用（需要异常上下文保存和恢复）或内核调用（不需要异常上下文保存和恢复）时，
 * 执行<thread_resched>相关函数
 * 
 * 例如，
 * thread_yield 
 * thread_block 
 ...
 * wait_queue_wake_all 
 * thread_exit
 * 只是这些函数都是同步句柄，在例程执行过程中，由例程自行调用
 * 
 * 系统调用并不是所有都需要触发调度
 * 用户和内核例程也可能会被 中断向量->异步句柄 触发调度
 * 
 */

static int idle_thread_routine(void *arg) {
    for (;;)
        arch_idle();
}
...
void thread_resched(void) {
    ...
    dprintf(ALWAYS, "arch_context_switch@self: cpu %d, old %p (%s, pri %d, flags 0x%x)\n",
            cpu, oldthread, oldthread->name, oldthread->priority,
            oldthread->flags);
#endif

#if defined(ARCH_HAS_MPU) && defined(CONFIG_RTOSPARTITION) && defined (CONFIG_MULTIPARTITIONING)
        if (!(newthread->flags & THREAD_FLAG_PARTITION))
		    return;
        
	    /* 需要RTOS提供当前任务以及任务切换函数 */
	    extern unsigned int *app_info_header;

	    void *(*curr_thx_func)(void) = app_info_header[6];
        void (*switch_func)(void) = app_info_header[7];

	    void *oldcurr_thx = curr_thx_func();
#if DEBUG_THREAD_CONTEXT_SWITCH        
        dprintf(ALWAYS, "%p, %p\n", curr_thx_func, oldcurr_thx);
#endif
	    switch_func();

        void *newcurr_thx = curr_thx_func();

#if DEBUG_THREAD_CONTEXT_SWITCH        
        dprintf(ALWAYS, "%p, %p\n", curr_thx_func, newcurr_thx);
#endif        

        if (oldcurr_thx == 0 ||
            newcurr_thx == 0 || 
            oldcurr_thx == newcurr_thx ||
            (*((addr_t *)oldcurr_thx)) == 0 ||
            (*((addr_t *)newcurr_thx)) == 0)
	        return;
        /* do the low level context switch */
	    arch_context_switch_pls(newthread, oldcurr_thx, newcurr_thx);
#endif /* ARCH_HAS_MPU && CONFIG_RTOSPARTITION && CONFIG_MULTIPARTITIONING */
	    return;
    }
    ...
    sys_bigtime_t now = current_time_hires();
...
/* 
 * one slot以timer为单位进行触发中断，timer可以是固定周期，也可以偶发一次
 * periodic以时间片为单位进行触发中断，中断的产生只是固定周期
 * 
 */
#if PLATFORM_HAS_DYNAMIC_TIMER
...
#ifdef WITH_HYPER_MODE
    if (newthread->flags & THREAD_FLAG_VM)
        vcpu_switch((oldthread->flags & THREAD_FLAG_VM) ? &oldthread->arch : NULL, 
            &newthread->arch);
#endif

#if defined(ARCH_HAS_MPU) && defined(CONFIG_MULTIPARTITIONING)
#ifdef __KERNEL_64__
    z_arm64_swap_mem_domains(newthread);
#else
    z_arm_configure_dynamic_mpu_regions(newthread); 
#endif
#endif
...
enum handler_return thread_timer_tick(struct timer *t, sys_time_t now, void *arg) {
    thread_t *current_thread = get_current_thread();
...
static enum handler_return thread_sleep_handler(timer_t *timer, sys_time_t now, void *arg) {
...
void thread_init_early(void) {
    ...
    k_mem_domain_add_thread(&k_mem_domain_default, t);
#endif
}
...
void thread_become_idle(void) {
    ...
    // TODO 32
    // comment "idle_thread_routine(t);" can trigger assert fail:
    // "thread_init_early" idle_thread->entry not enter;
    // according to "arm_context_switch", the idle thread uses "kernel-percpu" stack
    // so, we need to fix "arm_context_switch" r0 to 'fixed' base addr, and change it to idle stack;
    // but the way makes "bootstrap2" thread run in abort state
    // 1. "thread_become_idle DEBUG_ASSERT(arch_ints_disabled());"
    // 2. "thread_secondary_cpu_init_early DEBUG_ASSERT(arch_ints_disabled());"
    // 3. undefined abort, halting

    // TODO 64, not switched to idle task, than bootstrap2 task, thread_resched stop.
    idle_thread_routine(t);
    __UNREACHABLE;
}
...
static enum handler_return wait_queue_timeout_handler(timer_t *timer, sys_time_t now, void *arg) {
 ...
 */
status_t wait_queue_block(wait_queue_t *wait, sys_time_t timeout) {
    timer_t timer;
...
// the elf binary for kern or the ability to parse them.
