...
usage:
    ...
    if (!strcmp(argv[1].str, "list")) {
        ...
        bio_close(dev);
    } else if (!strcmp(argv[1].str, "memcreate")) {
	    void *memptr = memalign(BLOCKSIZE, argv[3].u);
        if (!memptr)
            rc = -1;
        else {
	        int err = create_membdev(argv[2].str, memptr, argv[3].u);
	        rc = err;
        }
