...
status_t watchdog_init(watchdog_t *dog, sys_time_t timeout, const char *name);
 ...
 * automatically dismiss a platform's HW watchdog using OLDK timers.  Platforms
 * must supply
 *
 ...
 * enable the HW watchdog.  SW watchdogs are based on OLDK timers, and should be
 ...
 * something managed to break timers on OLDK.
 */

extern status_t platform_watchdog_init(sys_time_t  target_timeout,
...
status_t watchdog_hw_init(sys_time_t timeout);
