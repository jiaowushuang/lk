...
bool net_timer_set(net_timer_t *t, net_timer_callback_t cb, void *callback_args, sys_time_t delay) {
...
static sys_time_t net_timer_work_routine(void) {
    sys_time_t now = current_time();
    sys_time_t delay = INFINITE_TIME;
...
static int net_timer_work_thread(void *args) {
    for (;;) {
        ...
        sys_time_t delay = net_timer_work_routine();
        if (delay != INFINITE_TIME) {
