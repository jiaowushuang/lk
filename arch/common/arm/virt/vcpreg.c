...
static bool vtimer_emulate(struct cpu_user_regs *regs, union hsr hsr) {return true;}
static bool vgic_emulate(struct cpu_user_regs *regs, union hsr hsr) {return true;}
...
/*
 * type32_t is defined as uint32_t due to the vreg_emulate_cp32 and
 * vreg_emulate_sysreg taking function pointer with uint32_t type used for
 * passing register's value.
 */
typedef uint32_t type32_t;
 ...
 */
#ifndef IS_64BIT
TVM_REG32(TTBR0_32, TTBR0)
TVM_REG32(TTBR1_32, TTBR1)
#else
TVM_REG32(TTBR0_32, TTBR0_EL1)
TVM_REG32(TTBR1_32, TTBR1_EL1)
#endif
TVM_REG64(TTBR0_64, TTBR0_EL1)
TVM_REG64(TTBR1_64, TTBR1_EL1)
...
#define GENERATE_TID3_INFO(reg, field, offset)                     \
  case HSR_CPREG32(reg): {                                         \
    return handle_ro_read_val(regs, regidx, cp32.read, hsr, 1, 0); \
  }
// TODO: guest_cpuinfo.field.bits[offset] to '0'

/* helper to define cases for all registers for one CRm value */
#define HSR_CPREG32_TID3_CASES(REG)     \
  case HSR_CPREG32(p15, 0, c0, REG, 0): \
...
void do_cp15_32(struct cpu_user_regs *regs, const union hsr hsr) {
  ...
  switch (hsr.bits & HSR_CP32_REGS_MASK) {
     ...
     */
    case HSR_CPREG32(CNTPCTL):
    case HSR_CPREG32(CNTPTVAL):
      if (!vtimer_emulate(regs, hsr)) return inject_undef_exception(regs, hsr);
      break;
    ...
    case HSR_CPREG32(ACTLR):
      if (psr_mode_is_user(regs)) return inject_undef_exception(regs, hsr);
      if (cp32.read) set_user_reg(regs, regidx, vcpu_read_reg(v, CTX_ACTLR));
      break;
     ...
     */
    default:
      dprintf(ALWAYS, "%s p15, %d, r%d, cr%d, cr%d, %d @ 0x%x\n",
              cp32.read ? "mrc" : "mcr", cp32.op1, cp32.reg, cp32.crn, cp32.crm,
              cp32.op2, regs->pc);
      dprintf(ALWAYS, "unhandled 32-bit CP15 access %lx\n",
...
void do_cp15_64(struct cpu_user_regs *regs, const union hsr hsr) {
  ...
  switch (hsr.bits & HSR_CP64_REGS_MASK) {
     ...
     */
    case HSR_CPREG64(ICC_SGI1R_EL1_64):
    case HSR_CPREG64(ICC_ASGI1R_EL1_64):
    case HSR_CPREG64(ICC_SGI0R_EL1_64):
      if (!vgic_emulate(regs, hsr)) return inject_undef_exception(regs, hsr);
      break;

      GENERATE_CASE(TTBR0_64, 64)
      GENERATE_CASE(TTBR1_64, 64)
    ...
    default: {
      ...
      dprintf(ALWAYS, "%s p15, %d, r%d, r%d, cr%d @ 0x%x\n",
              cp64.read ? "mrrc" : "mcrr", cp64.op1, cp64.reg1, cp64.reg2,
              cp64.crm, regs->pc);
      dprintf(ALWAYS, "unhandled 64-bit CP15 access %lx\n",
...
void do_cp14_32(struct cpu_user_regs *regs, const union hsr hsr) {
  ...
  switch (hsr.bits & HSR_CP32_REGS_MASK) {
    ...
    case HSR_CPREG32(DBGDIDR): {
       ...
       */
      if (!cp32.read) return inject_undef_exception(regs, hsr);
     ...
     */
    default:
      dprintf(ALWAYS, "%s p14, %d, r%d, cr%d, cr%d, %d @ 0x%x\n",
              cp32.read ? "mrc" : "mcr", cp32.op1, cp32.reg, cp32.crn, cp32.crm,
              cp32.op2, regs->pc);
      dprintf(ALWAYS, "unhandled 32-bit cp14 access %lx\n",
              hsr.bits & HSR_CP32_REGS_MASK);
      inject_undef_exception(regs, hsr);
      return;
  }
...
void do_cp14_64(struct cpu_user_regs *regs, const union hsr hsr) {
   ...
   * And all other unknown registers.
   */
  dprintf(ALWAYS, "%s p14, %d, r%d, r%d, cr%d @ 0x%x\n",
          cp64.read ? "mrrc" : "mcrr", cp64.op1, cp64.reg1, cp64.reg2, cp64.crm,
          regs->pc);
  dprintf(ALWAYS, "unhandled 64-bit CP14 access %lx\n",
          hsr.bits & HSR_CP64_REGS_MASK);
  inject_undef_exception(regs, hsr);
}
...
void do_cp14_dbg(struct cpu_user_regs *regs, const union hsr hsr) {
  ...
  dprintf(ALWAYS, "%s p14, %d, r%d, r%d, cr%d @ 0x%x\n",
          cp64.read ? "mrrc" : "mcrr", cp64.op1, cp64.reg1, cp64.reg2, cp64.crm,
          regs->pc);
  dprintf(ALWAYS, "unhandled 64-bit CP14 DBG access %lx\n",
          hsr.bits & HSR_CP64_REGS_MASK);
...
void do_cp10(struct cpu_user_regs *regs, const union hsr hsr) {
  ...
  switch (hsr.bits & HSR_CP32_REGS_MASK) {
    ...
    default:
      dprintf(ALWAYS, "%s p10, %d, r%d, cr%d, cr%d, %d @ 0x%x\n",
              cp32.read ? "mrc" : "mcr", cp32.op1, cp32.reg, cp32.crn, cp32.crm,
              cp32.op2, regs->pc);
      dprintf(ALWAYS, "unhandled 32-bit CP10 access %lx\n",
              hsr.bits & HSR_CP32_REGS_MASK);
      inject_undef_exception(regs, hsr);
      return;
  }
...
void do_cp(struct cpu_user_regs *regs, const union hsr hsr) {
  ...
  DEBUG_ASSERT(!cp.tas); /* We don't trap SIMD instruction */
  dprintf(ALWAYS, "unhandled CP%d access\n", cp.coproc);
  inject_undef_exception(regs, hsr);
}
