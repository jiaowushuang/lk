...
typedef bool (*vreg_reg64_fn_t)(struct cpu_user_regs *regs, uint64_t *r,
                                   bool read);
typedef bool (*vreg_reg_fn_t)(struct cpu_user_regs *regs, u_register_t *r,
                                   bool read);
 ...
 */
static inline u_register_t vreg_reg_extract(unsigned long reg,
                                          unsigned int offset,
                                          enum dabt_size size)
{
    reg >>= 8 * offset;
...
static inline void vreg_reg_update(unsigned long *reg, u_register_t val,
                                   unsigned int offset,
                                   enum dabt_size size)
{
...
static inline void vreg_reg_setbits(unsigned long *reg, u_register_t bits,
                                    unsigned int offset,
                                    enum dabt_size size)
{
    unsigned long mask = VREG_REG_MASK(size);
...
static inline void vreg_reg_clearbits(unsigned long *reg, u_register_t bits,
                                      unsigned int offset,
                                      enum dabt_size size)
{
    unsigned long mask = VREG_REG_MASK(size);
...
static inline u_register_t vreg_reg##sz##_extract(uint##sz##_t reg,       \
