...
static inline struct thread *arch_get_current_thread(void) {
    return (struct thread *)get_cpu_data(user);
}

static inline void arch_set_current_thread(struct thread *t) {
    set_cpu_data(user, (uintptr_t)t);
}

static inline void *arch_get_current_kstack(void) {
    return (void *)get_cpu_data(stack);
}

static inline struct world *arch_get_current_world_context(uint32_t security_state) {
    return (struct world *)get_cpu_data(world[security_state]);
}

static inline void arch_set_current_world_context(uint32_t security_state, struct world *w) {
    set_cpu_data(world[security_state], w);
}
