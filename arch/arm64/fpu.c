...
// static fp_regs_t *current_fpstate[SMP_MAX_CPUS];
static struct fpstate *current_fpstate[SMP_MAX_CPUS];

static void arm64_fpu_load_state(struct thread *t) {
    uint cpu = arch_curr_cpu_num();
#if 0    
    fp_regs_t *fpstate = (fp_regs_t *)(t->arch.tcpu_sp + CTX_GPREGS_END);

    if (fpstate == current_fpstate[cpu] && read_ctx_reg(fpstate, CTX_FP_CPU) == cpu) {
        LTRACEF("cpu %d, thread %s, fpstate already valid\n", cpu, t->name);
        return;
    }
    LTRACEF("cpu %d, thread %s, load fpstate %p, last cpu %lld, last fpstate %p\n",
            cpu, t->name, fpstate, read_ctx_reg(fpstate, CTX_FP_CPU), current_fpstate[cpu]);

    write_ctx_reg(fpstate, CTX_FP_CPU, cpu);
    current_fpstate[cpu] = fpstate;

    uint64_t fpcr_reg, fpsr_reg;

    fpcr_reg = read_ctx_reg(fpstate, CTX_FP_FPCR);
    fpsr_reg = read_ctx_reg(fpstate, CTX_FP_FPSR);

    fpregs_context_restore(fpstate);

    LTRACEF("thread %s, fpcr %llx, fpsr %llx\n", t->name, fpcr_reg, fpsr_reg);
#endif
...
void arm64_fpu_save_state(struct thread *t) {
    ...
    LTRACEF("frame %lx, fpstate %p\n", t->arch.tcpu_sp, fpstate);
    fpregs_context_save(fpstate);
    fpcr_reg = read_ctx_reg(fpstate, CTX_FP_FPCR);
    fpsr_reg = read_ctx_reg(fpstate, CTX_FP_FPSR);
#endif    
    struct fpstate *fpstate = &t->arch.fpstate;
    ...
    LTRACEF("thread %s, fpcr %lx, fpsr %lx\n", t->name, fpstate->fpcr, fpstate->fpsr);
}
