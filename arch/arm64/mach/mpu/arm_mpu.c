...
#include <mach/mpu/arch_helpers.h>
...
#define _MAX_DYNAMIC_MPU_REGIONS_NUM                                                               \
	((IS_ENABLED(CONFIG_MULTIPARTITIONING) ? (CONFIG_MAX_DOMAIN_PARTITIONS + 1) : 0) +                 \
...
#define MPU_DYNAMIC_REGIONS_AREA_START ((uintptr_t)&__data_start)
#else
#define MPU_DYNAMIC_REGIONS_AREA_START ((uintptr_t)&__data_start)
#endif
#define MPU_DYNAMIC_REGIONS_AREA_SIZE                                                             \
	((size_t)((uintptr_t)&_end_of_ram - MPU_DYNAMIC_REGIONS_AREA_START))
...
void z_arm64_mm_init(bool is_primary_core)
{
	...
	val = read_currentel();
	DEBUG_ASSERT_MSG(GET_EL(val) == MODE_EL1,
	...
	if ((val != ID_AA64MMFR0_PMSA_EN) &&
		...
		DEBUG_ASSERT_MSG(0, "MPU not supported!\n");
		return;
	}
	...
	LTRACEF("total region count: %d", get_num_regions());
	...
	if (rc <= 0) {
		DEBUG_ASSERT_MSG(0, "Dynamic areas init fail");
		return;
	}
#endif
...
static int dup_dynamic_regions(struct dynamic_region_info *dst, int len)
{
	...
	if (!(sys_dyn_regions_num < len)) {
		LTRACEF("system dynamic region nums too large.");
		ret = -EINVAL;
		goto out;
	}
			 ...
			 struct arm_mpu_region_attr *attr)
{
	...
	if (!(region_idx < region_num)) {
		LTRACEF("Out-of-bounds error for dynamic region map. "
	...
	if (!(u_idx >= 0)) {
		LTRACEF("Invalid underlying region index");
		ret = -ENOENT;
		goto out;
	}
					...
					uint8_t region_num)
{
	...
	for (size_t i = 0; i < region_num; i++) {
		...
		if (!(region_idx < get_num_regions())) {
			LTRACEF("Out-of-bounds error for mpu regions. "
...
int configure_dynamic_mpu_regions(struct thread *thread)
{
	/*
	...
	if (mem_domain) {
		LTRACEF("configure domain: %p", mem_domain);
		...
		for (size_t i = 0; i < max_parts && num_parts > 0; i++, num_parts--) {
			...
			LTRACEF("set region 0x%lx 0x%lx",
...
#if 0
	LTRACEF("configure user thread %p's context", thread);
...
void z_arm64_thread_mem_domains_init(struct thread *thread)
{
	configure_dynamic_mpu_regions(thread);
}

void z_arm64_swap_mem_domains(struct thread *thread)
{
	configure_dynamic_mpu_regions(thread);
}
