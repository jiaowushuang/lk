...
#define DWT_CYCCNT (0xE0001004)
    ...
    return 0;
#endif
#elif ARM_ISA_ARMV7 || ARM_ISA_ARMV8
    uint32_t count;
...
#if WITH_SMP && (ARM_ISA_ARMV7 || ARM_ISA_ARMV8)
...
static inline struct thread *arch_get_current_thread(void) {
    ...
    return (struct thread *)read_htpidr(); 
#else
...
extern uint8_t init_stack[ARCH_DEFAULT_STACK_SIZE * SMP_MAX_CPUS];
static inline void *arch_get_current_kstack(void) {
    uint core = arch_curr_cpu_num();
    return (void *)(init_stack + core * ARCH_DEFAULT_STACK_SIZE);
}
