	...
	const struct nxp_mpu_region *region_conf)
{
	...
	LTRACEF("[%02d] 0x%08x 0x%08x 0x%08x 0x%08x\n", index,
	...
	k_mem_partition_attr_t *new_attr)
{
	...
	DEBUG_ASSERT(reg_index != get_num_regions(),
	...
	const uint32_t background_area_end)
{
	if (mpu_configure_static_mpu_regions(static_regions, regions_num,
		...
		DEBUG_ASSERT(0, "Configuring %u static MPU regions failed\n",
			regions_num);
	}
}
	...
	const struct z_arm_mpu_partition dynamic_regions[], uint8_t regions_num)
{
	if (mpu_configure_dynamic_mpu_regions(dynamic_regions,
		...
		DEBUG_ASSERT(0, "Configuring %u dynamic MPU regions failed\n",
			regions_num);
	}
}
