...
extern void arm_core_mpu_set_vmid(uint8_t vmid);
#endif
extern unsigned int __data_start, _end_of_ram;
/*
 ...
 */
#if defined(CONFIG_MULTIPARTITIONING)
#define _MPU_DYNAMIC_REGIONS_AREA_START ((uint32_t)&__data_start)
#define _MPU_DYNAMIC_REGIONS_AREA_SIZE ((uint32_t)&_end_of_ram - \
...
#if !defined(CONFIG_MULTIPARTITIONING) && defined(CONFIG_MPU_STACK_GUARD)
extern uint8_t initial_stack[];
#endif

#if defined(CONFIG_FPU) && defined(CONFIG_FPU_SHARING) \
	&& defined(CONFIG_MPU_STACK_GUARD)
extern uint32_t z_arm_mpu_stack_guard_and_fpu_adjust(struct thread *thread);
#endif
...
static const struct z_arm_mpu_partition static_regions[] = {
#if defined(CONFIG_COVERAGE_GCOV) && defined(CONFIG_MULTIPARTITIONING)
		{
...
#endif /* CONFIG_COVERAGE_GCOV && CONFIG_MULTIPARTITIONING */
		 ...
		 * under CONFIG_MULTIPARTITIONING=n, so the MPU guard (if
		 ...
		 */
		{
			.start = (uint32_t)initial_stack,
			.size = (uint32_t)MPU_GUARD_ALIGN_AND_SIZE,
			.attr = K_MEM_PARTITION_P_RO_U_NA,
		},
#endif /* !CONFIG_MULTIPARTITIONING && CONFIG_MPU_STACK_GUARD */
};
...
void z_arm_configure_static_mpu_regions(void)
{
	...
	arm_core_mpu_configure_static_mpu_regions(static_regions,
		...
		(uint32_t)&_end_of_ram);
#ifdef ARM_ISA_ARMV8R
	arm_core_mpu_enable();
#endif

#if defined(CONFIG_MPU_REQUIRES_NON_OVERLAPPING_REGIONS) && \
	defined(CONFIG_MULTIPARTITIONING)
 ...
 */
void z_arm_configure_dynamic_mpu_regions(struct thread *thread)
{
	 ...
	 *
	 * The drivers of what regions get configured are CONFIG_MULTIPARTITIONING,
	 * CONFIG_MPU_STACK_GUARD, and K_USER/supervisor threads.
	 *
	 * If CONFIG_MULTIPARTITIONING is defined and the thread is a member of any
	 * memory domain then any partitions defined within that domain get a
	 * defined region.
	 *
	 * If CONFIG_MULTIPARTITIONING is defined and the thread is a user thread
...
#if defined(CONFIG_MULTIPARTITIONING)
	/* Memory domain */
	LTRACEF("configure thread %p's domain\n", thread);
	struct k_mem_domain *mem_domain = thread->mem_domain_info.mem_domain;

	if (mem_domain) {
		LTRACEF("configure domain: %p\n", mem_domain);
		...
		for (i = 0; i < CONFIG_MAX_DOMAIN_PARTITIONS; i++) {
			...
			LTRACEF("set region 0x%lx 0x%x\n",
				partition->start, partition->size);
			DEBUG_ASSERT_MSG(region_num < _MAX_DYNAMIC_MPU_REGIONS_NUM,
...
#if 0	
	/* Thread user stack */
	LTRACEF("configure user thread %p's context\n", thread);
	if (thread->arch.priv_stack_start) {
		...
		DEBUG_ASSERT(region_num < _MAX_DYNAMIC_MPU_REGIONS_NUM,
			"Out-of-bounds error for dynamic region map.");
	...
	if (thread->arch.priv_stack_start) {
		...
		DEBUG_ASSERT((uintptr_t)&z_priv_stacks_ram_start <= guard_start,
...
#endif /* CONFIG_MULTIPARTITIONING */
	{
		...
		DEBUG_ASSERT((uintptr_t)thread->stack_obj == guard_start,
...
#ifdef WITH_HYPER_MODE
	/* Configurate VMID */
	arm_core_mpu_set_vmid(thread->mem_domain_info.vmid);
#endif	
	arm_core_mpu_enable();
#endif
}
