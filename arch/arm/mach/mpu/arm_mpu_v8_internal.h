...
#include <mach/cmse/cmse.h>

/* ARMV8M - ARM_CPU_CORTEX_M23/ARM_CPU_CORTEX_M33/ARM_CPU_CORTEX_M55
 * ARMV8R - ARM_ISA_ARMV8R
 */
...
#if defined(ARM_CPU_CORTEX_M23) || defined(ARM_CPU_CORTEX_M33) || \
...
static inline void mpu_set_renr(uint32_t renr)
{
	write_hprenr(renr);
	DSB;
	ISB;
...
/* An alias of the HPRLAR[31:0].EN bits */
static inline uint32_t mpu_get_renr(void)
{
	return read_hprenr();
}

static inline void mpu_set_vmid(uint8_t vmid)
{
	write_vsctlr(VSCTLR_VMID(vmid));
}

static inline uint8_t mpu_get_num_regions(void)
{
	uint32_t type = read_hmpuir();

	type &= MPU_IR_REGION_Msk;
	...
	const struct arm_mpu_region *region_conf)
{
	...
	LTRACEF("[%d] 0x%08x 0x%08x 0x%08x 0x%08x\n",
...
static inline int get_region_index(uint32_t start, uint32_t size)
{
	...
	for (uint8_t idx = 0; idx < mpu_get_num_regions(); idx++) {
		mpu_set_rnr(idx);
		uint32_t rbar = mpu_get_rbar() & MPU_RBAR_BASE_Msk;
		uint32_t rlar = mpu_get_rlar() & MPU_RLAR_LIMIT_Msk;
		LTRACEF("idx %d, rbar %x, rlar %x\n", idx, rbar, rlar);
		if (start >= rbar && limit <= rlar) {
			return idx;
		}
	}
	return -EINVAL;
...
#endif /* ARM_CPU_HAS_TEE */
	return -EPERM;
}
#endif /* ARM_ISA_ARMV8R */

#endif /* CONFIG_MULTIPARTITIONINGTIONING */
	...
	bool do_sanity_check)
{
	...
	for (i = 0; i < regions_num; i++) {
		...
		if ((u_reg_index == -EINVAL) ||
			...
			LTRACEF("Invalid underlying region index %u\n",
				u_reg_index);
			return -EINVAL;
		}
