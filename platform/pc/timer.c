...
sys_time_t current_time(void) {
    ...
    // XXX slight race
    time = (sys_time_t) (timer_current_time >> 32);
...
sys_bigtime_t current_time_hires(void) {
    ...
    // XXX slight race
    time = (sys_bigtime_t) ((timer_current_time >> 22) * 1000) >> 10;
...
static enum handler_return os_timer_tick(void *arg) {
    ...
    sys_time_t time = current_time();
    //sys_bigtime_t btime = current_time_hires();
