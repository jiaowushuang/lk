...
/*
 * This uses the RTC1 module for the OLDK system tick instead of using the ARM
...
//delta to be added to base counter (ms in 49.15 format) each kern tick
...
status_t platform_set_periodic_timer(platform_timer_callback callback, void *arg, sys_time_t interval) {
    ...
    nrf_rtc_cc_set(NRF_RTC1, RTC_CC_CHAN, rtc_counts_per_tick);
    ...
    nrf_rtc_int_enable(NRF_RTC1, RTC_CHANNEL_INT_MASK(RTC_CC_CHAN));

    // In order for this event to wake the device from low power/sleep state, the
    // event must be enabled to do so.
    nrf_rtc_event_enable(NRF_RTC1, RTC_CHANNEL_INT_MASK(RTC_CC_CHAN));

    // Clear the event state prior to enabling the irq to prevent spurious irq
    nrf_rtc_event_clear(NRF_RTC1, nrf_rtc_compare_event_get(RTC_CC_CHAN));
...
void nrf52_RTC1_IRQ(void) {
    ...
    last_cc = nrf_rtc_cc_get(NRF_RTC1, RTC_CC_CHAN);
    nrf_rtc_cc_set(NRF_RTC1, RTC_CC_CHAN, last_cc + rtc_counts_per_tick);

    bool resched = false;
    if (cb) {
        sys_time_t now = DIV_BY_32768(base_counter);
