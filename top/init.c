...
#define TRACE_INIT (SYS_DEBUGLEVEL >= 2)
#ifndef EARLIEST_TRACE_LEVEL
#define EARLIEST_TRACE_LEVEL INIT_LEVEL_TARGET_EARLY
#endif

extern const struct bootstrap_init_struct __start_bootstrap_init __WEAK;
extern const struct bootstrap_init_struct __stop_bootstrap_init __WEAK;

void bootstrap_init_level(enum bootstrap_init_flags required_flag, uint start_level, uint stop_level) {
    ...
    const struct bootstrap_init_struct *last = NULL;
    for (;;) {
        /* search for the lowest uncalled hook to call */
        LTRACEF("last %p, last_called_level %#x\n", last, last_called_level);

        const struct bootstrap_init_struct *found = NULL;
        bool seen_last = false;
        for (const struct bootstrap_init_struct *ptr = &__start_bootstrap_init; ptr != &__stop_bootstrap_init; ptr++) {
...
INIT_HOOK(test, test_hook, 1);
INIT_HOOK(test1, test_hook1, 1);
INIT_HOOK(test2, test_hook2, 2);
INIT_HOOK(test1a, test_hook1a, 1);
INIT_HOOK(test1b, test_hook1b, 1);
#endif
