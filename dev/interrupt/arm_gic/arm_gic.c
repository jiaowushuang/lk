...
#if WITH_LIB_SM || WITH_HYPER_MODE
...
/* virtual cpu regs */
#define GICV_CTLR               (VGICC_OFFSET + 0x0000)
#define GICV_PMR                (VGICC_OFFSET + 0x0004)
#define GICV_BPR                (VGICC_OFFSET + 0x0008)
#define GICV_IAR                (VGICC_OFFSET + 0x000c)
#define GICV_EOIR               (VGICC_OFFSET + 0x0010)
#define GICV_RPR                (VGICC_OFFSET + 0x0014)
#define GICV_HPPIR              (VGICC_OFFSET + 0x0018)
#define GICV_APBR               (VGICC_OFFSET + 0x001c)
#define GICV_AIAR               (VGICC_OFFSET + 0x0020)
#define GICV_AEOIR              (VGICC_OFFSET + 0x0024)
#define GICV_AHPPIR             (VGICC_OFFSET + 0x0028)
#define GICV_APR(n)             (VGICC_OFFSET + 0x00d0 + (n) * 4)
#define GICV_NSAPR(n)           (VGICC_OFFSET + 0x00e0 + (n) * 4)
#define GICV_IIDR               (VGICC_OFFSET + 0x00fc)
#define GICV_DIR                (VGICC_OFFSET + 0x1000)
...
static void arm_gic_init_percpu(uint level) {
#if WITH_LIB_SM || WITH_HYPER_MODE
    GICREG(0, GICC_CTLR) = 0xf; // enable GIC0 and select fiq mode for secure, enable ackctl
    /* If the highest priority pending interrupt is a Group 1 interrupt, a read of the GICC_IAR
    ...
    */
    ...
    GICREG(0, GICD_IGROUPR(0)) = 0U; /* GICD_IGROUPR0(0-31 int num) is banked */
#else
...
static void arm_vgic_init_percpu(uint level) {
    GICREG(0, GICV_CTLR) = 1; // enable VGIC0
    GICREG(0, GICV_PMR) = 0xFF; // unmask interrupts at all priority levels
}
#endif

INIT_HOOK_FLAGS(arm_gic_init_percpu,
                   arm_gic_init_percpu,
                   INIT_LEVEL_PLATFORM_EARLY, INIT_FLAG_SECONDARY_CPUS);
#ifdef WITH_HYPER_MODE                   
INIT_HOOK_FLAGS(arm_vgic_init_percpu,
                   arm_vgic_init_percpu,
                   INIT_LEVEL_PLATFORM_EARLY, INIT_FLAG_SECONDARY_CPUS);
#endif
...
INIT_HOOK_FLAGS(arm_gic_suspend_cpu, arm_gic_suspend_cpu,
                   INIT_LEVEL_PLATFORM, INIT_FLAG_CPU_SUSPEND);
...
INIT_HOOK_FLAGS(arm_gic_resume_cpu, arm_gic_resume_cpu,
...
void arm_gic_init(void) {
    ...
    // Initialize all the SPIs priority
    for (i = 32; i < MAX_INT; i += 4) {
        //u_int reg = i / 4;
#ifdef WITH_HYPER_MODE
        //GICREG(0, GICD_IPRIORITYR(reg)) = 0x80808080;
#else
        //GICREG(0, GICD_IPRIORITYR(reg)) = 0;
#endif    
    }
...
#if WITH_LIB_SM || WITH_HYPER_MODE
__WEAK enum handler_return sm_handle_irq(void) {
	return INT_NO_RESCHEDULE;
}
#endif
...
volatile struct gich_vcpu_ctrl_map *gic_vcpu_ctrl =
    (volatile struct gich_vcpu_ctrl_map *)(GICBASE(0)+VGICD_OFFSET);
#endif

unsigned int gic_vcpu_num_list_regs;

#endif /* End of WITH_HYPER_MODE */
